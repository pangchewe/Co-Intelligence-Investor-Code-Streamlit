...
st.markdown(
    """
    **Credits**  
    This application was developed with insights from [Pang (QuantCorner)](https://www.linkedin.com/in/dhouch/),[AJ.Pat (investidea.in.th)](https://web.facebook.com/investidea.in.th) [P.Prem(DataKarate)](https://web.facebook.com/datakarate/?_rdc=1&_rdr#) and [P.Nut(QuantCorner)](https://web.facebook.com/quantcornerthailand).
    """
)
...
2.2 บริษัทควรโตใกล้เคียงกับอุตสาหกรรม ถ้าบริษัทโตเร็วกว่าอุตสาหกรรม ไม่นานการเติบโตจะตันและค่อยๆกลับมาอุตสาหกรรม ถ้าบริษัทเติบโตช้ากว่าอุตสาหกรรมไม่นานคู่แข่งจะเติบโตกว่าและชิงส่วนแบ่งการตลาดไป
...
prompt7 = template.safe_substitute(
    ...
    business_type=business_type,
    task="""
2.4 ลักษณะของงบการเงินที่เติบโต อย่างมีคุณภาพ
...
2.4.4 อัตราส่วน หนี้สินต่อทุนสม่ำเสมอแสดงว่าจัดวางโครงสร้างทางการเงินได้ดี
2.5 ลัษณะงบการเงินของหุ้นที่กำลังเติบโตลดลง
2.5.1 ROA เริ่มลดลงต่อเนื่องหลายปี
2.5.1 อัตราส่วนหมุนเวียนสินทรัพย์ลดลง จากสินทรัพย์เพิ่มแล้วรายได้ไม่เพิ่มตาม
...
3.2 หุ้นที่ PE Ratio สูงเกิน 20  แสดงว่า ตลาดให้ความสนใจกับการเติบโตในอนาคต มากกว่ากำไรในปัจจุบัน
3.2 หุ้นที่ PE Ratio ต่ำกว่า 20 แสดงว่า ตลาดให้ความสนใจกับกำไรในปัจจุบัน ให้ระวัง
3.2.1 หุ้น PE Ratio เพราะกำไรที่มาครั้งเดียว เช่นกำไรจากการขายสินทรัพย์ กำไรจากอัตราแลกเปลี่ยน 
3.2.2 หุ้น PE Ratio เพราะกำไรจากราคาสินค้าพุ่ง อาจมาแค่ครั้งเดียวแล้วก็หาย
3.2.2 หุ้น PE Ratio เพราะกำไรผันผวนไม่สม่ำเสมอ ตลาดต้องให้ PE ต่ำเพื่อชดเชยความเสี่ยงที่กำไรผลผวน
""",
)

# Get the response from Gemini
output1 = get_gemini_completion(prompt)
output2 = get_gemini_completion(prompt2)
output3 = get_gemini_completion(prompt3)
output4 = get_gemini_completion(prompt4)
